                    ...
                    int attach)
{
    ...
    switch(attach) {
        case ATTACH_TYPE_TYPEC:
            /*Tab A9 code for AX6739A-2728 by wenyaqi at 20230906 start*/
            schedule_delayed_work(&upm->force_detect_dwork, msecs_to_jiffies(150));
            /*Tab A9 code for AX6739A-2728 by wenyaqi at 20230906 end*/
            break;
...
static int upm6910x_enable_hz(struct charger_device *chg_dev, bool enable)
{
    struct upm6910x_device *upm = charger_get_data(chg_dev);

    dev_info(upm->dev, "%s value = %d\n", __func__, enable);
    if (enable) {
        upm6910x_enable_hiz_mode(upm);
    } else {
        upm6910x_disable_hiz_mode(upm);
    }
