...
int aw_retry_src_caps(int slate_mode)
{
    struct aw35615_chip *chip = aw35615_GetChip();
    AW_U32 bak_cur;
    AW_U32 cur;
    ...
    if (slate_mode == SLATE_0A) {
        cur = 0; //0A
    } else if (slate_mode == SLATE_500MA) {
        cur = 500; //500mA
    } else {
        cur = 1200; //1200mA
    }
    ...
    if ((!chip->queued) && (chip->port.PolicyState == peSourceReady)) {
        ...
        AW_LOG("queue_work --> send source cap,cur=%d\n",cur);
        do {
...
EXPORT_SYMBOL(aw_retry_src_caps);
...
void handle_core_event(AW_U32 event, AW_U8 portId, void *usr_ctx, void *app_ctx)
{
    ...
    case CC_NO_ORIENT:
        ...
        //when plug out usb, send PD_CONNECT_NONE event
        tcpci_notify_pd_state(chip->tcpc, PD_CONNECT_NONE);
        //when plug out usb, set sink vbus = 0v 0a;
        tcpci_sink_vbus(chip->tcpc, TCP_VBUS_CTRL_PD | TCP_VBUS_CTRL_PD_DETECT,0, 0);
        AW_LOG("aw35615 - when plug out usb, set sink vbus = 0v 0a\n");
    ...
    case PD_NEW_CONTRACT:
        AW_LOG("aw35615 :PD_NEW_CONTRACT=0x%x, PE_ST=%d\n",
                event, chip->port.PolicyState);
        /*Tab A9 code for AX6739A-224 | AX6739A-708 by wenyaqi at 20230606 start*/
        // 10 : Maximum Operating Current 10mA units.
        // 20 : Output Voltage in 20mV units.
        // 50 : Operating Current 50mA units.
        if (chip->port.SrcCapsReceived[chip->port.SinkRequest.FVRDO.ObjectPosition - 1].PDO.SupplyType ==
            pdoTypeAugmented) {
        ...
        /*Tab A9 code for AX6739A-224 | AX6739A-708 by wenyaqi at 20230606 end*/
        break;
    case DATA_ROLE:
